    ...
    def __init__(self, *args, product=None, **kwargs):
        ...
        if product:
            ...
            self.fields['quantity'].validators.append(
                MaxValueValidator(
                    product.stock,
                    message=f'在庫は{product.stock}個までです'
                )
            )
    ...
    def clean_quantity(self):
        ...
        if hasattr(self, 'product'):
            if quantity > self.product.stock:
                raise forms.ValidationError(f'在庫が不足しています（在庫: {self.product.stock}個）')
        return quantity
...
class CheckoutForm(forms.Form):
    ...
    def clean_shipping_postal_code(self):
        ...
        if not postal_code.isdigit() or len(postal_code) != 7:
            raise forms.ValidationError('郵便番号は7桁の数字で入力してください（例: 1234567 または 123-4567）')
        # ハイフン付きで返す
