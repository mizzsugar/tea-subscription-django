...
from model.models import User, Tea, FavoriteTea, TeaReview, TeaProduct, Order, OrderItem, TaxRate, ShippingFee, Cart, CartItem
...
class ShippingFeeAdmin(admin.ModelAdmin):
    list_display = ['fee', 'free_shipping_threshold', 'start_date', 'is_active', 'created_at']
    list_filter = ['is_active', 'start_date']
    ordering = ['-start_date']
...
class TeaProductAdmin(admin.ModelAdmin):
    list_display = ['tea', 'weight', 'price', 'get_price_with_tax', 'stock', 'is_available']
...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        ...
        'shipping_fee', 'total_amount', 'tax_rate',
    ...
    fieldsets = (
        ...
        ('金額', {
            'fields': ('subtotal', 'tax_rate', 'tax_amount', 'shipping_fee', 'total_amount')
        }),
        ('配送先', {
